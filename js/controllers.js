...
.controller('MenuCtrl', function($http, $scope, $sce, $ionicScrollDelegate,$timeout,$rootScope){
	...
	$http.get("http://thinkingmindpro.com/api/get_category_index/").then(
...
.controller('MainCtrl', function($http, $scope, $sce, $ionicScrollDelegate,$timeout, $localStorage, $ionicLoading){
	...
	$scope.doRefresh = function(){
		$scope.recent_posts = [];
		$http.get("http://thinkingmindpro.com/api/get_posts/").then(function(data){
			console.log(data);
	...
	$scope.loadMore = function(){
		...
		{
			$scope.lastTimer = new Date().getTime();
			$http.get("http://thinkingmindpro.com/api/get_posts/?offset="+$scope.offset)
			.then(function(data){
...
.controller('PostCtrl', function($scope, $http, $stateParams, $sce){
	...
	$http.get('http://thinkingmindpro.com/api/get_post/?id='+ $stateParams.postId).then(
		function(data){
...
.controller('CatCtrl', function($http, $scope, $sce, $stateParams){
	...
	$scope.doRefresh = function(){
		$http.get('http://thinkingmindpro.com/api/get_category_posts/?id=' + $stateParams.catId).then(
		function(data){
...
.controller('FavCtrl', function($http, $scope, $localStorage, $sce){
    ...
    $scope.Favorites.forEach(function(element, index, array){
      $http.get('http://thinkingmindpro.com/api/get_post/?id='+element)
